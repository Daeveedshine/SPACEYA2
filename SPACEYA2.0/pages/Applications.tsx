...
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({
  ...
  <div className="space-y-6 break-inside-avoid">
    <h3 className="text-xs font-black text-zinc-400 uppercase tracking-[0.4em] border-b border-zinc-100 dark:border-zinc-800 pb-2">
      {title}
    </h3>
    {children}
  </div>
...
const PrintRow: React.FC<{ label: string; value: any }> = ({
  label,
  value,
}) => (
  <div className="mb-4">
    ...
    },
    {
      ...
      fields: [
        ...
        },
        {
          ...
          label: "Name of Current Landlord",
          type: "text",
          required: true,
        },
        {
          id: "f13",
          key: "currentLandlordPhone",
      ...
      fields: [
        {
          ...
          type: "select",
          options: ["NIN", "Voter's Card", "Passport", "Drivers License"],
          required: true,
        },
        {
          id: "f15",
          key: "verificationIdNumber",
          label: "ID Number",
          type: "text",
          required: true,
        },
  ...
  const validateAgent = (id: string) => {
    ...
    setTimeout(() => {
      const agent = store.users.find(
        (u) =>
          u.id.toLowerCase() === id.toLowerCase() && u.role === UserRole.AGENT,
      );
      if (agent) {
        ...
        <div className="bg-white dark:bg-zinc-900 p-10 md:p-14 rounded-[3.5rem] shadow-2xl border border-zinc-100 dark:border-zinc-800 max-w-lg w-full text-center space-y-8">
          ...
          <div>
            ...
            <p className="text-zinc-500 font-medium mt-2 text-sm">
              Enter the unique ID of your leasing agent to access their specific
              enrollment form.
            </p>
          </div>
          <div className="relative">
            ...
            <input
              ...
              onKeyDown={(e) =>
                e.key === "Enter" && validateAgent(targetAgentId)
              }
            />
          </div>
          <button
          ...
          <div className="pt-6 border-t border-zinc-100 dark:border-zinc-800">
            <button
              ...
              className="text-zinc-400 font-bold text-xs hover:text-blue-600 transition-colors uppercase tracking-widest"
            >
      ...
      <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in duration-500 pb-20">
        <div className="flex items-center justify-between">
          <button
            ...
            className="p-3 bg-white dark:bg-zinc-900 rounded-2xl hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
          >
            <ArrowLeft size={20} className="text-zinc-500" />
          </button>
          <div className="text-center">
            <h2 className="text-lg font-black text-zinc-900 dark:text-white uppercase tracking-tighter">
              Application for {targetAgent?.name}
            </h2>
            <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">
              Step {currentStepIndex + 1} of {activeTemplate.sections.length}
            </p>
          </div>
        ...
        <div className="flex gap-2">
          {activeTemplate.sections.map((_, idx) => (
            <div
              key={idx}
              className={`h-1.5 flex-1 rounded-full transition-all duration-700 ${idx <= currentStepIndex ? "bg-blue-600" : "bg-zinc-200 dark:bg-zinc-800"}`}
            />
          ))}
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {currentSection.fields.map((field) => (
                ...
                >
                  ...
                  {field.type === "textarea" ? (
                    <textarea
                      className="glass-input w-full p-6 rounded-[2rem] text-sm font-bold h-32 resize-none outline-none focus:ring-2 focus:ring-blue-600 text-zinc-900 dark:text-white"
                    ...
                    <div className="relative">
                      <select
                        className="glass-input w-full p-6 rounded-[2rem] text-sm font-bold appearance-none outline-none focus:ring-2 focus:ring-blue-600 text-zinc-900 dark:text-white"
                        value={formData[field.key] || ""}
                        onChange={(e) =>
                          handleInputChange(field.key, e.target.value)
                        }
                      >
                        <option
                          value=""
                          className="text-zinc-900 dark:text-white bg-white dark:bg-zinc-900"
                        >
                          Select Option...
                        </option>
                    ...
                    <div className="relative">
                      <input
                        type="date"
                        className="glass-input w-full p-6 rounded-[2rem] text-sm font-bold outline-none focus:ring-2 focus:ring-blue-600 text-zinc-900 dark:text-white block"
                        value={formData[field.key] || ""}
                        onChange={(e) =>
                          handleInputChange(field.key, e.target.value)
                        }
                    ...
                    <input
                      type={field.type}
                      className="glass-input w-full p-6 rounded-[2rem] text-sm font-bold outline-none focus:ring-2 focus:ring-blue-600 text-zinc-900 dark:text-white"
                      value={formData[field.key] || ""}
                      onChange={(e) =>
                        handleInputChange(field.key, e.target.value)
                      }
            ...
            <div className="flex gap-4 pt-6 border-t border-zinc-100 dark:border-zinc-800">
              {currentStepIndex > 0 && (
                <button
                  ...
                  className="flex-1 bg-offwhite dark:bg-zinc-800 py-6 rounded-[2rem] font-black uppercase text-[10px] text-zinc-400"
                >
                  Back
                </button>
              )}
              {isLastStep ? (
                ...
                <button
                  onClick={() => setCurrentStepIndex((prev) => prev + 1)}
                  className="flex-[2] bg-blue-600 text-white py-6 rounded-[2rem] font-black uppercase tracking-widest text-[11px] shadow-xl shadow-blue-600/20 flex items-center justify-center gap-3"
                >
    ...
    <div className="space-y-6 print:hidden-container">
      ...
      {myApplications.length > 0 ? (
        myApplications.map((app) => (
          <div
            key={app.id}
            className="bg-white dark:bg-zinc-900 p-8 rounded-[2.5rem] border border-zinc-100 dark:border-zinc-800 flex flex-col md:flex-row items-center justify-between shadow-sm group hover:border-blue-200 transition-all gap-6 print:hidden"
          >
        ...
        <div className="bg-white dark:bg-zinc-900 p-16 rounded-[4rem] border-2 border-dashed border-zinc-100 dark:border-zinc-800 text-center space-y-8 shadow-sm print:hidden">
          ...
          <div className="space-y-2">
            ...
            <p className="text-zinc-400 text-sm max-w-xs mx-auto font-medium">
              No tenancy dossiers found on your registry. Begin your enrollment
              process to start a new lifecycle.
            </p>
          </div>
        </div>
      )}
      ...
      {viewingApp && (
        <div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-300 overflow-hidden">
          <div className="bg-white dark:bg-zinc-900 w-full max-w-4xl max-h-[90dvh] rounded-[3rem] shadow-2xl overflow-hidden flex flex-col border border-zinc-200 dark:border-zinc-800 relative">
            ...
            <div
              ...
              className="p-10 overflow-y-auto custom-scrollbar space-y-12 bg-white dark:bg-zinc-900 text-black dark:text-white"
            >
              <div className="flex flex-col md:flex-row gap-8 items-center border-b border-zinc-100 dark:border-zinc-800 pb-10">
                <div className="w-24 h-24 bg-zinc-100 dark:bg-black rounded-3xl overflow-hidden border-2 border-zinc-200 dark:border-zinc-800">
                  {viewingApp.passportPhotoUrl ? (
                    <img
                      src={viewingApp.passportPhotoUrl}
                      className="w-full h-full object-cover"
                      alt="Profile"
                    />
                  ) : (
                ...
                <div className="text-center md:text-left">
                  <h2 className="text-3xl font-black text-zinc-900 dark:text-white tracking-tighter">
                    {viewingApp.firstName} {viewingApp.surname}
                  </h2>
                  <div className="flex items-center justify-center md:justify-start gap-4 mt-2">
                    <span className="px-4 py-1 rounded-full text-[10px] font-black uppercase bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400">
                      {viewingApp.status}
                    </span>
                    <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">
                      Date:{" "}
                      {new Date(viewingApp.submissionDate).toLocaleDateString()}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <Section title="01: Registry Information">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                  ...
                  <PrintRow
                    label="Marital Status"
                    value={viewingApp.maritalStatus}
                  />
              ...
              <Section title="02: Residential Analysis">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <PrintRow
                    ...
                    value={viewingApp.currentHomeAddress}
                  />
                  <PrintRow
                    label="Relocation Reason"
                    value={viewingApp.reasonForRelocating}
                  />
                  <PrintRow
                    label="Landlord"
                    value={viewingApp.currentLandlordName}
                  />
                  <PrintRow
                    label="Landlord Phone"
                    value={viewingApp.currentLandlordPhone}
                  />
                </div>
              </Section>
              ...
              <Section title="03: Verification Dossier">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <PrintRow
                    label="ID Type"
                    value={viewingApp.verificationType}
                  />
                  <PrintRow
                    label="ID Number"
                    value={viewingApp.verificationIdNumber}
                  />
                </div>
              ...
              {Object.keys(viewingApp.customResponses || {}).length > 0 && (
                <Section title="04: Additional Information">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {Object.entries(viewingApp.customResponses || {}).map(
                      ([key, value]) => {
                        if (
                          key === "agentIdCode" ||
                          key === "signature" ||
              ...
              <div className="pt-8 border-t border-zinc-100 dark:border-zinc-800 text-center">
                ...
                <p className="text-4xl font-serif italic text-zinc-900 dark:text-white mb-2">
                  {viewingApp.signature}
                </p>
                <p className="text-[8px] font-bold text-zinc-300 uppercase tracking-widest">
                  Digital Signature Verified • {viewingApp.applicationDate}
                </p>
              </div>
            </div>
          </div>
