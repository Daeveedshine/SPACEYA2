  ...
  const handleRouteProperty = (propertyId: string) => {
    ...
    setTimeout(() => {
      ...
      const updatedUsers = store.users.map((u) =>
        u.id === selectedApp.userId
          ? { ...u, assignedPropertyId: propertyId }
          : u,
      );
      ...
      const propertyName = store.properties.find(
        (p) => p.id === propertyId,
      )?.name;
      const notification = {
      ...
      setSelectedApp(
        updatedApplications.find((a) => a.id === selectedApp.id) || null,
      );
      setIsRouting(false);
  ...
  const getStatusStyle = (status: ApplicationStatus) => {
    switch (status) {
      ...
      case ApplicationStatus.PENDING:
        return "bg-amber-500/10 text-amber-500 border-amber-600/20";
      default:
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-24">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 px-1 print:hidden">
        ...
        <div className="relative w-full sm:w-auto">
          ...
          <input
            ...
            className="glass-input w-full sm:w-64 pl-12 pr-4 py-4 rounded-2xl text-sm outline-none font-bold shadow-xl"
            value={searchTerm}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-4 max-h-[calc(100dvh-250px)] overflow-y-auto pr-2 custom-scrollbar print:hidden">
          {filteredApps.map((app) => (
            <button
              key={app.id}
              onClick={() => setSelectedApp(app)}
              className={`w-full text-left p-6 rounded-[2.5rem] border-2 transition-all flex items-center gap-6 ${selectedApp?.id === app.id ? "bg-blue-600 text-white border-blue-400 shadow-2xl" : "glass-card border-zinc-200 dark:border-zinc-800 shadow-sm"}`}
            >
              <div
                className={`w-16 h-16 rounded-2xl overflow-hidden flex items-center justify-center font-black shrink-0 border shadow-xl ${selectedApp?.id === app.id ? "bg-blue-500 border-blue-400" : "bg-zinc-100 dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700"}`}
              >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p
                  className={`text-[10px] font-bold uppercase tracking-widest ${selectedApp?.id === app.id ? "text-blue-100" : "text-zinc-500"}`}
                >
                  {app.status}
                </p>
              </div>
          ...
          {filteredApps.length === 0 && (
            <div className="text-center py-20 glass-card rounded-[3rem] border-2 border-dashed border-zinc-200 dark:border-zinc-800 text-zinc-400">
              No records found.
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-2">
          {selectedApp ? (
            <div
              ...
              className="bg-white dark:bg-zinc-900 rounded-[4rem] shadow-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 animate-in slide-in-from-right-8 duration-700 print:shadow-none print:rounded-none print:border-none print:m-0 print:p-0 print:overflow-visible print:w-full"
            >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] font-bold uppercase tracking-[0.3em] text-blue-400">
                      Official Tenancy Enrollment Form
                    </p>
                  </div>
                </div>
              ...
              <div className="bg-zinc-900 p-12 text-white flex flex-col md:flex-row justify-between items-center gap-8 border-b-8 border-blue-600 print:bg-transparent print:text-black print:border-none print:p-12 print:pt-0">
                <div className="flex items-center gap-8">
                  <div>
                    <h2 className="text-4xl font-black tracking-tighter print:text-black">
                      {selectedApp.firstName} {selectedApp.surname}
                    </h2>
                    <div className="flex items-center gap-4 mt-2">
                      <span
                        className={`px-4 py-1 rounded-full text-[10px] font-black uppercase border print:border-zinc-300 print:text-black ${getStatusStyle(selectedApp.status)}`}
                      >
                        {selectedApp.status}
                      </span>
                      <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">
                        Application Date: {selectedApp.applicationDate}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="p-10 md:p-14 space-y-16 text-black print:p-12 print:pt-4">
                ...
                {selectedApp.status === ApplicationStatus.APPROVED &&
                  !store.users.find((u) => u.id === selectedApp.userId)
                    ?.assignedPropertyId && (
                    <section className="p-8 bg-blue-600/5 border-2 border-blue-600/20 rounded-[3rem] space-y-8 animate-in zoom-in-95 duration-500 print:hidden">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h4 className="text-xl font-black text-zinc-900 tracking-tight">
                              Lifecycle Asset Routing
                            </h4>
                            <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">
                              Route an available property to this candidate
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {availableProperties.map((prop) => (
                          ...
                          >
                            <div
                              className={`p-3 rounded-2xl ${routedPropertyId === prop.id ? "bg-blue-600 text-white" : "bg-zinc-50 text-zinc-400"}`}
                            >
                              <Building size={20} />
                            </div>
                            <div className="min-w-0">
                              <p className="font-black text-zinc-900 tracking-tight truncate">
                                {prop.name}
                              </p>
                              ...
                              <p className="text-xs font-black text-blue-600 mt-1">
                                ₦{prop.rent.toLocaleString()}/yr
                              </p>
                            </div>
                            {routedPropertyId === prop.id && (
                              <Check
                                size={20}
                        ...
                        {availableProperties.length === 0 && (
                          <div className="col-span-full py-12 text-center text-zinc-400 opacity-60">
                            ...
                            <p className="text-[10px] font-black uppercase tracking-widest">
                              No available assets found in your registry
                            </p>
                          </div>
                        )}
                      </div>
                      ...
                      {routedPropertyId && (
                        <div className="pt-4 animate-in slide-in-from-bottom-4">
                          <button
                            onClick={() =>
                              handleRouteProperty(routedPropertyId)
                            }
                            disabled={isRouting}
                          ...
                          >
                            ...
                            {isRouting
                              ? "routing lifecycle..."
                              : "Confirm Routing & Activate Tenancy"}
                          </button>
                        </div>
                      )}
                    </section>
                ...
                <section className="space-y-10 break-inside-avoid">
                  <h3 className="text-xs font-black text-zinc-400 uppercase tracking-[0.4em] border-b-2 border-zinc-100 pb-4 flex items-center gap-3">
                    <UserIcon size={16} className="text-blue-600" /> 01: Profile
                    Information
                  </h3>
                ...
                <section className="space-y-10 break-inside-avoid">
                  <h3 className="text-xs font-black text-zinc-400 uppercase tracking-[0.4em] border-b-2 border-zinc-100 pb-4 flex items-center gap-3">
                    <MapPin size={16} className="text-blue-600" /> 02:
                    Residential Analysis
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <DetailRow
                      label="Current Residential Address"
                      value={selectedApp.currentHomeAddress}
                    />
                    <DetailRow
                      label="Primary Reason for Relocation"
                      value={selectedApp.reasonForRelocating}
                    />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10 p-8 bg-zinc-50 rounded-[2.5rem] border border-zinc-100 print:bg-transparent print:border-zinc-200">
                    <DetailRow
                      ...
                      value={selectedApp.currentLandlordName}
                    />
                    <DetailRow
                      label="Landlord Mobile Number"
                      value={selectedApp.currentLandlordPhone}
                    />
                  </div>
                </section>
                ...
                <section className="space-y-10 break-inside-avoid">
                  <h3 className="text-xs font-black text-zinc-400 uppercase tracking-[0.4em] border-b-2 border-zinc-100 pb-4 flex items-center gap-3">
                    <ShieldCheck size={16} className="text-blue-600" /> 03:
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    <div className="space-y-10">
                      <div className="grid grid-cols-2 gap-6">
                        <DetailRow
                          ...
                          value={selectedApp.verificationType}
                        />
                        <DetailRow
                          label="Document Reference"
                          value={selectedApp.verificationIdNumber}
                        />
                      </div>
                    </div>
                    <div className="p-14 bg-zinc-950 rounded-[4rem] flex flex-col items-center justify-center text-center shadow-2xl print:bg-white print:border-4 print:rounded-none print:border-zinc-200">
                      <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.5em] mb-8 print:text-zinc-400">
                ...
                {Object.keys(selectedApp.customResponses || {}).length > 0 && (
                  <section className="space-y-10 break-inside-avoid">
                    <h3 className="text-xs font-black text-zinc-400 uppercase tracking-[0.4em] border-b-2 border-zinc-100 pb-4 flex items-center gap-3">
                      <List size={16} className="text-blue-600" /> 04:
                      Additional Information
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                      {Object.entries(selectedApp.customResponses || {}).map(
                        ([key, value]) => {
                          if (
                            key === "agentIdCode" ||
                            key === "signature" ||
                ...
                <div className="pt-12 border-t border-zinc-100 flex flex-col sm:flex-row gap-6 print:hidden">
                  {selectedApp.status !== ApplicationStatus.APPROVED && (
                    <button
                      ...
                      className="flex-[2] bg-emerald-600 hover:bg-black text-white py-6 rounded-[2rem] font-black uppercase text-xs flex items-center justify-center gap-3 transition-all shadow-xl shadow-emerald-100 active:scale-95"
                    >
                  ...
                  <button
                    ...
                    className="flex-1 bg-rose-50 border-2 border-rose-100 text-rose-500 py-6 rounded-[2rem] font-black uppercase text-xs flex items-center justify-center gap-3 transition-all hover:bg-rose-500 hover:text-white active:scale-95"
                  >
            ...
            <div className="h-[calc(100vh-250px)] flex flex-col items-center justify-center bg-zinc-950 rounded-[4rem] border-2 border-dashed border-zinc-900 print:hidden">
              ...
              <p className="text-zinc-700 font-black uppercase tracking-[0.3em] text-sm">
                Select Dossier to Review
              </p>
            </div>
          )}
        </div>
...
const DetailRow: React.FC<{ label: string; value: any }> = ({
  ...
  <div className="min-w-0 mb-6 print:mb-4">
    <p className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.3em] leading-none mb-2 truncate print:text-zinc-600">
      {label}
    </p>
    <p className="text-lg font-bold text-black leading-tight break-words">
      {value || "N/A"}
    </p>
  </div>
);
